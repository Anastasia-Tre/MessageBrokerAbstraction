...
{
    internal MemoryMessageBrokerBuilder(MessageBrokerBuilder other) :
        base(other)
    {
    }

    private static string DefaultMessageTypeToTopicConverter(Type type)
    {
        return type.Name;
    }

    public MemoryMessageBrokerBuilder AutoDeclareFrom(
        IEnumerable<Assembly> assemblies,
        Func<Type, bool> subscriberTypeFilter = null,
        Func<Type, string> messageTypeToTopicConverter = null)
    {
        ...
        var prospectTypes = assemblies
            ...
            .Where(x =>
                x.GetInterfaces().First().IsGenericType &&
                x.GetInterfaces().First().GetGenericTypeDefinition() ==
                typeof(ISubscriber<>))
            .Where(x => subscriberTypeFilter == null || subscriberTypeFilter(x))
            .Where(x => !x.IsGenericType || x.IsConstructedGenericType);

        var foundSubscribers =
            prospectTypes; //.Where(x => x.GetInterfaces().First().GetGenericTypeDefinition() == typeof(ISubscriber<>)).ToList();

        var knownMessageTypes = new HashSet<Type>();

        // register all subscribers, handlers
        foreach (var subscriber in foundSubscribers)
            knownMessageTypes.Add(subscriber.GetInterfaces().First()
        ...
        foreach (var messageType in knownMessageTypes)
        {
            ...
            var isRoot = messageTypeAncestors.All(ancestor =>
                !knownMessageTypes.Contains(ancestor));
            if (isRoot)
            {
        ...
        {
            var subscribers = foundSubscribers.Where(x =>
                x.GetInterfaces().First().GetGenericArguments().First() ==
                rootMessageType
                || ancestorsByType[
                        x.GetInterfaces().First().GetGenericArguments().First()]
                    .Contains(rootMessageType)).ToList();
            if (subscribers.Count > 0)
            {
                ...
                SetSubscriber(rootMessageType, x =>
                {
                    ...
                    foreach (var subscriber in subscribers)
                        if (subscriber.GetInterfaces().First()
                                .GetGenericArguments().First() ==
                            rootMessageType &&
                            !x.SubscriberSettings.Invokers.Contains(
                                x.SubscriberSettings))
    ...
    private static ISet<Type> GetAncestorTypes(Type messageType)
    {
        ...
        for (var mt = messageType;
             mt.BaseType != typeof(object) && mt.BaseType != null;
             mt = mt.BaseType) ancestors.Add(mt.BaseType);
        return ancestors;
    }
}
