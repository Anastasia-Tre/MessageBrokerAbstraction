...
namespace Provider.Redis
{
    internal class RedisSubscriber : IDisposable
    {
        ...
        public RedisSubscriber(SubscriberSettings settings, ISubscriber subscriber, IMessageHandler<byte[]> messageProcessor)
        {
            _channelMessageQueue = subscriber.Subscribe(settings.Topic);
            _channelMessageQueue.OnMessage(m => messageProcessor.HandleMessage(m.Message));
        }
